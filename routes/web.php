...
Route::middleware(['middleware' => 'auth', 'checkCart'])->group(function () {
    ...
    // PayPal gateway
    // Route::post('/generate-phonepe-qr', [PaymentController::class, 'generatePhonePeQRCode']);
    // Route::post('/phonepe/callback', [PaymentController::class, 'phonePeCallback'])->name('phonepe.callback');
    Route::post('/payment/initiate', [PaymentController::class, 'initiatePayment'])->name('payment.initiate');

    // Payment callback (handles PhonePe's response)
    Route::get('/payment/callback', [PaymentController::class, 'paymentCallback'])->name('payment.callback');

    // Payment status check
    Route::get('/payment/status/{transactionId}', [PaymentController::class, 'checkPaymentStatus'])->name('payment.status');
